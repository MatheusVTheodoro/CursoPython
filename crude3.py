...
class Ticket:
    def __init__(self, cliente, problema, prioridade):
...
def adicionar_ticket():
    ...
    if cliente and problema.strip() and prioridade:
        novo_ticket = Ticket(cliente, problema, prioridade)
...
def atualizar_lista_tickets():
    ...
    for i, ticket in enumerate(tickets):
        lista_tickets.insert(tk.END, f"Ticket {i+1}: Cliente: {ticket.cliente} | Prioridade: {ticket.prioridade}")
...
def exibir_descricao_ticket(event):
    ...
    if indice_selecionado:
        ...
        label_cliente = tk.Label(janela_descricao, text=f"Cliente: {ticket_selecionado.cliente}")
        label_cliente.pack()

        label_problema = tk.Label(janela_descricao, text="Descrição do Problema:")
        label_problema.pack()
        ...
        label_prioridade = tk.Label(janela_descricao, text=f"Prioridade: {ticket_selecionado.prioridade}")
...
label_cliente = tk.Label(frame_formulario, text="Cliente:")
label_cliente.grid(row=0, column=0, padx=5)
entry_cliente = tk.Entry(frame_formulario)
entry_cliente.grid(row=0, column=1, padx=5)

label_problema = tk.Label(frame_formulario, text="Problema:")
...
label_prioridade = tk.Label(frame_formulario, text="Prioridade:")
...
combo_prioridade_menu = tk.OptionMenu(frame_formulario, combo_prioridade, *prioridades)
combo_prioridade_menu.grid(row=2, column=1, padx=5)

btn_adicionar = tk.Button(frame_formulario, text="Adicionar", command=adicionar_ticket)
...
label_lista = tk.Label(frame_lista, text="Lista de Tickets:")
label_lista.pack()
